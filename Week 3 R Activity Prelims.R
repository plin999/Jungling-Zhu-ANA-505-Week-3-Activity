...
LingTitanic <- as.data.frame.table(Titanic)
LingTitanic
class(LingTitanic)
...
summary.data.frame(LingTitanic)
...
select(LingTitanic,2)
select(LingTitanic,3)
values_Sex <-LingTitanic$Sex
values_Age <- LingTitanic$Age
...
new_data <- data.frame(column1 = values_Sex, column2 = values_Age)
...
subset_data <- LingTitanic[LingTitanic$Freq > 25, ]
...
ggplot(LingTitanic, aes(Survived,Freq)) +
  geom_col()
